...
exports.showIndexPage = (req, res) => {
   ...
   mysqlPool.pool.getConnection((err, connection) => {
      ...
      query += "order by post_date desc limit 8;";

      query += "select p.prj_id, IFNULL(pc1.pj1,0) as pc1, IFNULL(pc2.pj2,0) as pc2, IFNULL(pc3.pj3,0) as pc3, ";
      query += "p.prj_name, p.prj_content, p.cls_date, p.start_date, p.keyword1, p.keyword2, p.keyword3, p.recommended_Prof, m.mentor_name, m.company_name,team_pj.team_name as team_id,";
      ...
      query +=
         "SELECT ads.term_chk, COUNT(ads.settings_id) as count from admin_settings as ads LEFT OUTER JOIN project on ads.settings_id=project.settings_id WHERE project.use_yn=1 GROUP BY ads.settings_id;";
      ...
      connection.query(query, null, (error, results, fields) => {
         ...
         res.render("index", { NoticePJImgList: results, userInfo: req.session.userInfo, moment: moment, curDate: new Date() });
      });
   });
};
