...
exports.getMakeproject = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query, null, (error, results, fields) => {
      ...
      res.render('pjmake/DGU301', {CodeTermList: results, userId: req.session.userId, userType: req.session.userType, userInfo: req.session.userInfo, moment: moment, curDate: new Date()});
    });
  });
};
...
exports.getManageproject = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query, null, (error, results, fields) => {
      ...
      //use results and fields
      res.render('pjmake/DGU311', {PJList: results, userId: req.session.userId, userType: req.session.userType, userInfo: req.session.userInfo, moment: moment, curDate: new Date()});
    });
  });
};
...
exports.getMentoringproject = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    query += " AND mentor_id = '"+ req.session.userId + "' order by prj_id desc ;";
    ...
    connection.query(query, null, (error, results, fields) => {
      ...
      //use results and fields
      res.render('pjmake/DGU302', {PJList: results, userId: req.session.userId, userType: req.session.userType,userInfo: req.session.userInfo, moment: moment, curDate: new Date()});
    });
  });
};
...
exports.postMentoringSchd = (req, res) => {
    ...
    var query ="";
    query += "update project set mentoring_schd = '"+mentoring_num+"' where prj_id = '" + req.body.PJId +"' ; " ;
    ...
    for (var i=1; i<=mentoring_num; i++){
      ...
      query += "insert into mentoring_schd (registrant, regis_date, mentor_id, prj_id, meeting_date, mentoring_topic) ";
      query += "values ('" +req.session.userId+ "', '"+moment(Date()).format('YYYY-MM-DD hh:mm:ss') +"', '"+ req.session.userId +"', '"+ req.body.PJId +"', '" +Mentoringdate+"', '" +MentoringDetail +"' ); ";
      query += " insert into mentoring_report (registrant, regis_date, amender, amend_date,  mentor_id, prj_id, meeting_date) ";
      query += "values ('" +req.session.userId+ "', '"+moment(Date()).format('YYYY-MM-DD hh:mm:ss') +"','"+req.session.userId+ "', '"+moment(Date()).format('YYYY-MM-DD hh:mm:ss')+"','"+ req.session.userId +"', '"+ req.body.PJId +"', '" +Mentoringdate+"'); ";
...
exports.getAdditionPj = (req, res) => {
  ...
  var query = "select p.*  from project as p where p.prj_id = '"+ req.params.pjId+"' ;";
  ...
  query +="select company_name,mentor_id ,mentor_name from mentor where auth_status='1' order by company_name ; ";
  //get connection from pool
  mysqlPool.pool.getConnection((err, connection) => {
    if(err) { //throw err;
      console.error('getConnection err : ' + err);
      ...
      connection.query(query, (error, results, fields) => {
        ...
        if(results.length > 0) {
          ...
          res.render('pjmake/DGU314', {PjSettingInfo: results, moment : moment, userInfo: req.session.userInfo});
        } else {
          res.redirect('/');
        }
      });
...
exports.postAdditionPj = (req, res) => {
  ...
  fileUpload(fileInfo).multipartForm(req, res, (err) => {
    ...
    if(req.body.preMat=="1" && req.body.PjName.substring(0,14) != "[Pre-Matching]"){
      prjName = "[Pre-Matching] " + req.body.PjName;
    }else if(req.body.preMat=="0" && req.body.PjName.substring(0,14) == "[Pre-Matching]"){
...
exports.getDetailPj = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query, (error, results, fields) => {
      ...
      if(results.length > 0) {
          logger.putLogDetail(req,'lookup project success.');
        res.render('pjmake/DGU312', {pjInfo: results, moment : moment, userInfo: req.session.userInfo});
      } else {
        res.redirect('/');
      }
    });
...
exports.getEditPj = (req, res) => {
  ...
  var query = "select p.*  from project as p where p.prj_id = '"+ req.body.projectID+"' ;";
  ...
  query += "select m.mentor_id, m.mentor_name, m.company_name from mentor as m; ";
  query += "select t.team_name,t.team_id,p.prj_name,p.prj_id from team as t left join project as p on t.prj_id = p.prj_id and t.use_yn='1'; ";
    ...
    if(err) { //throw err;
      ...
      connection.query(query, (error, results, fields) => {
        ...
        if(results.length > 0) {
          ...
          res.render('pjmake/DGU313', {PjSettingInfo: results, moment : moment, userType: req.session.userType,userInfo: req.session.userInfo});
        } else {
          res.redirect('/');
        }
      });
...
exports.postEditproject = (req, res) => {
  ...
  fileUpload(fileInfo).multipartForm(req, res, (err) => {
    ...
    mysqlPool.pool.getConnection((err, connection) => {
      ...
      var query2 = "update team set ? where team_id = '"+req.body.selectTeam+"';";
      var query3 = "update team set ? where team_id = '"+req.body.originTeam+"';";
...
//session check
  ...
  var query = "select p.prj_id, p.prj_name, p.mentor_id, m.mentor_name, m.company_name from project as p ";
  query += "left join mentor as m on p.mentor_id=m.mentor_id and p.use_yn=1 where p.use_yn = 1 ; ";

  query += "select mentor_id, company_name ,mentor_name from mentor order by company_name; ";
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query, (error, results, fields) => {
      ...
      //use results and fields
      res.render('pjmake/DGU315', {PjMentor: results, userInfo: req.session.userInfo});
    });
  });
};
