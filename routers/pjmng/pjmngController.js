  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query, (error, results, fields) => {
      ...
      if(results.length > 0) {
        ...
        res.render('pjmng/DGU501', {pjInfo: results, moment : moment, userInfo: req.session.userInfo});
      } else {
        res.redirect('/');
      }
    });
...
exports.postprjplan = (req,res)=>{
  ...
  fileUpload(fileInfo).multipartForm(req, res, (err) => {
    ...
    if(req.params.formType == 'PostReport') {
      ...
      logger.putLogDetail(req,'Delete project plan Report: ' + req.body.PJId + ', id: ' + req.session.userId);
      mysqlPool.pool.getConnection((err, connection) => {
        if(err) { //throw err;
          console.error('getConnection err : ' + err);
          return;
...
exports.getSearchproject1 = (req,res)=>{
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    if(req.session.userInfo.userType=='mentor'){
      query += "select p.*, s.settings_id, s.term_chk, t.team_name, t.prj_id, sti.*, st.major, st.email_ad, st.std_name, st.phone_num  from project as p, admin_settings as s, team as t, std_team_info as sti, student as st";
      query += " where p.settings_id = s.settings_id and p.use_yn = 1 and t.prj_id = p.prj_id and sti.team_id = t.team_id and p.mentor_id = '"+ req.session.userId+"' and st.std_id = sti.std_id";
      query += " order by p.prj_id;";
      // console.log(query);

    }
    else if(req.session.userInfo.userType=='student'){
      query += "select p.*, s.settings_id, s.term_chk, t.team_name, t.prj_id, sti.std_id, sti.team_id from project as p, admin_settings as s, team as t, std_team_info as sti";
      query += " where p.settings_id = s.settings_id and p.use_yn = 1 and t.prj_id = p.prj_id and sti.team_id = t.team_id and sti.std_id = '"+ req.session.userId+"'";
      query += " order by p.prj_id;";
      // console.log(query);
    }
    else if(req.session.userInfo.userType=='admin'){
      query = "select p.*, s.settings_id, s.term_chk, t.team_name, t.prj_id from project as p, admin_settings as s, team as t";
      query += " where p.settings_id = s.settings_id and p.use_yn = 1 and t.prj_id = p.prj_id";
      query += " order by p.prj_id;";
      // console.log(query);
...
exports.postSearchproject = (req, res) => {
  ...
  if (req.body.selectTerm != "0"){
    if (cnt!=0){query_where += " AND prj_term = '" + req.body.selectTerm +"'" ;}
    else {  query_where += " prj_term = '" + req.body.selectTerm +"'";}
    cnt ++ ;
  }

  if (req.body.selectDevelopment != "0"){
    if (cnt!=0){  query_where += " AND prj_dev_field = '" + req.body.selectDevelopment +"'" ;}
    else {query_where += " prj_dev_field = '" + req.body.selectDevelopment+"'" ;}
    cnt ++ ;
  }

  if (req.body.SearchContent !=''){
    if (cnt!=0){query_where += " AND prj_name LIKE '" + req.body.SearchContent + "%'" ;}
    else {  query_where += " prj_name LIKE '%" + req.body.SearchContent + "%'" ;}
    cnt ++ ;
  }
...
exports.getSearchproject = (req,res)=>{
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    if(req.session.userInfo.userType=='mentor'){
      query += "select p.*, s.settings_id, s.term_chk, t.team_name, t.prj_id from project as p, admin_settings as s, team as t";
      query += " where p.settings_id = s.settings_id and p.use_yn = 1 and t.prj_id = p.prj_id and p.mentor_id = '"+ req.session.userId+"'";
      query += " order by p.prj_id;";
      // console.log(query);
    ...
    connection.query(query, (error, results, fields) => {
      ...
      //use results and fields
      res.render('pjmng/DGU512', {PJList: results, userId: req.session.userId, userType: req.session.userType,userInfo: req.session.userInfo, moment: moment, curDate: new Date()});
...
exports.getSearchproject2 = (req,res)=>{
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query, (error, results, fields) => {
      ...
      //use results and fields
      res.render('pjmng/DGU522', {PJList: results, userId: req.session.userId, userType: req.session.userType,userInfo: req.session.userInfo, moment: moment, curDate: new Date()});
...
exports.getMentoringReport = (req,res)=>{
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    if(req.session.userInfo.userType=='student'){
      var query = "select m.*,sti.* from mentoring_report as m, std_team_info as sti";
      query += " where sti.team_id = m.team_id and sti.std_id = '"+ req.session.userId+"'";
      query += " order by m.sub_date;"
      query += "select t.*,sti.* from team as t, std_team_info as sti";
      query += " where sti.team_id = t.team_id and sti.std_id = '"+ req.session.userId+"';";
    }else if(req.session.userInfo.userType=='mentor'){
      var query = "select m.*,sti.* from mentoring_report as m, team as sti";
      query += " where sti.team_id = m.team_id and sti.mentor_id = '"+ req.session.userId+"'";
      query += " order by m.sub_date;"
    ...
    connection.query(query, (error, results, fields) => {
      ...
      if(results.length > 0) {
        ...
        res.render('pjmng/DGU511', {mtrInfo: results, moment : moment, userInfo: req.session.userInfo});
      } else {
        res.redirect('/');
      }
    });
...
exports.getMentoringReport1 = (req,res)=>{
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    if(req.session.userInfo.userType=='student'){
      query += "select sti.*,t.*,p.settings_id, ad.mentoring_limit from std_team_info as sti, project as p, team as t, admin_settings as ad"
      query += " where sti.std_id = '"+ req.session.userId+"'and t.prj_id = '"+ req.params.PJId+"' and p.prj_id = t.prj_id and t.team_id = sti.team_id and p.settings_id = ad.settings_id; ";
    }
    else{
...
exports.postMentoringReport1 = (req, res) => {
  ...
  if(req.session.userInfo.userType=='student'){
    ...
    if(req.params.PJId == 'DeleteReport1') {
      ...
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt1+"'";
      logger.putLogDetail(req,'Delete Mentoring Report: ' + req.body.MTDt1 + ', id: ' + req.session.userId);
    }else if(req.params.PJId == 'DeleteReport2') {
      var query = "delete from mentoring_report";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt2+"'";
      logger.putLogDetail(req,'Delete Mentoring Report: ' + req.body.MTDt2 + ', id: ' + req.session.userId);

    }else if(req.params.PJId == 'DeleteReport3') {
      var query = "delete from mentoring_report";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt3+"'";
        logger.putLogDetail(req,'Delete Mentoring Report: ' + req.body.MTDt3 + ', id: ' + req.session.userId);

    }else if(req.params.PJId == 'DeleteReport4') {
      var query = "delete from mentoring_report";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt4+"'";
      logger.putLogDetail(req,'Delete Mentoring Report: ' + req.body.MTDt4 + ', id: ' + req.session.userId);

    }else if(req.params.PJId == 'DeleteReport5') {
      var query = "delete from mentoring_report";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt5+"'";
      logger.putLogDetail(req,'Delete Mentoring Report: ' + req.body.MTDt5 + ', id: ' + req.session.userId);

    }else if(req.params.PJId == 'DeleteReport6') {
      var query = "delete from mentoring_report";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt6+"'";
      logger.putLogDetail(req,'Delete Mentoring Report: ' + req.body.MTDt6 + ', id: ' + req.session.userId);

    }else if(req.params.PJId == 'DeleteReport7') {
      var query = "delete from mentoring_report";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt6+"'";
      logger.putLogDetail(req,'Delete Mentoring Report: ' + req.body.MTDt7 + ', id: ' + req.session.userId);

    }else if(req.params.PJId == 'DeleteReport8') {
      var query = "delete from mentoring_report";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt7+"'";
      logger.putLogDetail(req,'Delete Mentoring Report: ' + req.body.MTDt8 + ', id: ' + req.session.userId);

    }else if(req.params.PJId == 'DeleteReport9') {
      var query = "delete from mentoring_report";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt8+"'";
      logger.putLogDetail(req,'Delete Mentoring Report: ' + req.body.MTDt9 + ', id: ' + req.session.userId);

    }else if(req.params.PJId == 'DeleteReport10') {
      var query = "delete from mentoring_report";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt10+"'";
      logger.putLogDetail(req,'Delete Mentoring Report: ' + req.body.MTDt10 + ', id: ' + req.session.userId);
    ...
    if(req.params.PJId == 'DeleteReport1') {
      ...
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt1+"'";
      logger.putLogDetail(req,'Update Mentoring Report: ' + req.body.MTDt1 + ', id: ' + req.session.userId);
    }else if(req.params.PJId == 'DeleteReport2') {
      var query = "update mentoring_report";
      query += " set ment_report_mtr = null";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt2+"'";
      logger.putLogDetail(req,'Update Mentoring Report: ' + req.body.MTDt2 + ', id: ' + req.session.userId);

    }else if(req.params.PJId == 'DeleteReport3') {
      var query = "update mentoring_report";
      query += " set ment_report_mtr = null";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt3+"'";
      logger.putLogDetail(req,'Update Mentoring Report: ' + req.body.MTDt3 + ', id: ' + req.session.userId);

    }else if(req.params.PJId == 'DeleteReport4') {
      var query = "update mentoring_report";
      query += " set ment_report_mtr = null";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt4+"'";
      logger.putLogDetail(req,'Update Mentoring Report: ' + req.body.MTDt4 + ', id: ' + req.session.userId);

    }else if(req.params.PJId == 'DeleteReport5') {
      var query = "update mentoring_report";
      query += " set ment_report_mtr = null";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt5+"'";
      logger.putLogDetail(req,'Update Mentoring Report: ' + req.body.MTDt5 + ', id: ' + req.session.userId);
    }else if(req.params.PJId == 'DeleteReport6') {
      var query = "update mentoring_report";
      query += " set ment_report_mtr = null";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt6+"'";
      logger.putLogDetail(req,'Update Mentoring Report: ' + req.body.MTDt6 + ', id: ' + req.session.userId);
    }else if(req.params.PJId == 'DeleteReport7') {
      var query = "update mentoring_report";
      query += " set ment_report_mtr = null";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt7+"'";
      logger.putLogDetail(req,'Update Mentoring Report: ' + req.body.MTDt7 + ', id: ' + req.session.userId);
    }else if(req.params.PJId == 'DeleteReport8') {
      var query = "update mentoring_report";
      query += " set ment_report_mtr = null";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt8+"'";
      logger.putLogDetail(req,'Update Mentoring Report: ' + req.body.MTDt8 + ', id: ' + req.session.userId);
    }else if(req.params.PJId == 'DeleteReport9') {
      var query = "update mentoring_report";
      query += " set ment_report_mtr = null";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt9+"'";
      logger.putLogDetail(req,'Update Mentoring Report: ' + req.body.MTDt9 + ', id: ' + req.session.userId);
    }else if(req.params.PJId == 'DeleteReport10') {
      var query = "update mentoring_report";
      query += " set ment_report_mtr = null";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt10+"'";
      logger.putLogDetail(req,'Update Mentoring Report: ' + req.body.MTDt10 + ', id: ' + req.session.userId);
    }
...
exports.postManagechk = (req, res) => {
  ...
  if(req.params.PJId == 'confirmchk1') {
    ...
    query += " set chk_yn = 1";
    query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt1+"'";
    logger.putLogDetail(req,'Update Payment check: ' + req.body.MTDt1 + ', id: ' + req.session.userId);
  }else if(req.params.PJId == 'confirmchk2') {
    var query = "update mentoring_report";
    query += " set chk_yn = 1";
    query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt2+"'";
    logger.putLogDetail(req,'Update Payment check: ' + req.body.MTDt2 + ', id: ' + req.session.userId);

  }else if(req.params.PJId == 'confirmchk3') {
    var query = "update mentoring_report";
    query += " set chk_yn = 1";
    query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt3+"'";
    logger.putLogDetail(req,'Update Payment check: ' + req.body.MTDt3 + ', id: ' + req.session.userId);

  }else if(req.params.PJId == 'confirmchk4') {
    var query = "update mentoring_report";
    query += " set chk_yn = 1";
    query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt4+"'";
    logger.putLogDetail(req,'Update Payment check: ' + req.body.MTDt4 + ', id: ' + req.session.userId);

  }else if(req.params.PJId == 'confirmchk5') {
    var query = "update mentoring_report";
    query += " set chk_yn = 1";
    query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt5+"'";
    logger.putLogDetail(req,'Update Payment check: ' + req.body.MTDt5 + ', id: ' + req.session.userId);

  }else if(req.params.PJId == 'confirmchk6') {
    var query = "update mentoring_report";
    query += " set chk_yn = 1";
    query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt6+"'";
    logger.putLogDetail(req,'Update Payment check: ' + req.body.MTDt6 + ', id: ' + req.session.userId);

  }else if(req.params.PJId == 'confirmchk7') {
    var query = "update mentoring_report";
    query += " set chk_yn = 1";
    query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt6+"'";
    logger.putLogDetail(req,'Update Payment check: ' + req.body.MTDt7 + ', id: ' + req.session.userId);

  }else if(req.params.PJId == 'confirmchk8') {
    var query = "update mentoring_report";
    query += " set chk_yn = 1";
    query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt7+"'";
    logger.putLogDetail(req,'Update Payment check: ' + req.body.MTDt8 + ', id: ' + req.session.userId);

  }else if(req.params.PJId == 'confirmchk9') {
    var query = "update mentoring_report";
    query += " set chk_yn = 1";
    query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt8+"'";
    logger.putLogDetail(req,'Update Payment check: ' + req.body.MTDt9 + ', id: ' + req.session.userId);

  }else if(req.params.PJId == 'confirmchk10') {
    var query = "update mentoring_report";
    query += " set chk_yn = 1";
    query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt10+"'";
    logger.putLogDetail(req,'Update Payment check: ' + req.body.MTDt10 + ', id: ' + req.session.userId);
...
exports.postMentoringReport4 = (req, res) => {
  ...
  var fileInfo = {
    ...
    namePrefix: 'MTR',
    viewNames: ['appendix1','appendix2','appendix3','appendix4','appendix5','appendix6','appendix7','appendix8','appendix9','appendix10']
  };
  ...
  fileUpload(fileInfo).multipartForm(req, res, (err) => {
    ...
    if(req.params.PJId == 'AddReport1') {
      var query = "update mentoring_report set ment_report_mtr = '" + req.files['appendix1'][0].path+"'";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt1+"'";
      console.log('Update Mentoring Report: ' + req.files['appendix1'][0].path + ', id: ' + req.session.userId);
      if(req.files['appendix1'] !== undefined) {
        mentoring_report.ment_report_mtr = req.files['appendix1'][0].path;
        console.log('submit Mentoring Report: ' + req.files['appendix1'][0].path + ', id: ' + req.session.userId);
        console.log("file upload success.");
      }
    }else if(req.params.PJId == 'AddReport2') {
      var query = "update mentoring_report set ment_report_mtr = '" + req.files['appendix2'][0].path+"'";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt2+"'";
      console.log('Update Mentoring Report: ' + req.files['appendix2'][0].path + ', id: ' + req.session.userId);
      if(req.files['appendix2'] !== undefined) {
        mentoring_report.ment_report_mtr = req.files['appendix2'][0].path;
        console.log('submit Mentoring Report: ' + req.files['appendix2'][0].path + ', id: ' + req.session.userId);
        console.log("file upload success.");
      }
    }else if(req.params.PJId == 'AddReport3') {
      var query = "update mentoring_report set ment_report_mtr = '" + req.files['appendix3'][0].path+"'";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt3+"'";
      console.log('Update Mentoring Report: ' + req.files['appendix3'][0].path + ', id: ' + req.session.userId);
      if(req.files['appendix3'] !== undefined) {
        mentoring_report.ment_report_mtr = req.files['appendix3'][0].path;
        console.log('submit Mentoring Report: ' + req.files['appendix3'][0].path + ', id: ' + req.session.userId);
        console.log("file upload success.");
      }
    }else if(req.params.PJId == 'AddReport4') {
      var query = "update mentoring_report set ment_report_mtr = '" + req.files['appendix4'][0].path+"'";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt4+"'";
      console.log('Update Mentoring Report: ' + req.files['appendix4'][0].path + ', id: ' + req.session.userId);
      if(req.files['appendix4'] !== undefined) {
        mentoring_report.ment_report_mtr = req.files['appendix4'][0].path;
        console.log('submit Mentoring Report: ' + req.files['appendix4'][0].path + ', id: ' + req.session.userId);
        console.log("file upload success.");
      }
    }else if(req.params.PJId == 'AddReport5') {
      var query = "update mentoring_report set ment_report_mtr = '" + req.files['appendix5'][0].path+"'";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt5+"'";
      console.log('Update Mentoring Report: ' + req.files['appendix5'][0].path + ', id: ' + req.session.userId);
      if(req.files['appendix5'] !== undefined) {
        mentoring_report.ment_report_mtr = req.files['appendix5'][0].path;
        console.log('submit Mentoring Report: ' + req.files['appendix5'][0].path + ', id: ' + req.session.userId);
        console.log("file upload success.");
      }
    }else if(req.params.PJId == 'AddReport6') {
      var query = "update mentoring_report set ment_report_mtr = '" + req.files['appendix6'][0].path+"'";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt6+"'";
      console.log('Update Mentoring Report: ' + req.files['appendix6'][0].path + ', id: ' + req.session.userId);
      if(req.files['appendix6'] !== undefined) {
        mentoring_report.ment_report_mtr = req.files['appendix6'][0].path;
        console.log('submit Mentoring Report: ' + req.files['appendix6'][0].path + ', id: ' + req.session.userId);
        console.log("file upload success.");
      }
    }else if(req.params.PJId == 'AddReport7') {
      var query = "update mentoring_report set ment_report_mtr = '" + req.files['appendix7'][0].path+"'";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt7+"'";
      console.log('Update Mentoring Report: ' + req.files['appendix7'][0].path + ', id: ' + req.session.userId);
      if(req.files['appendix7'] !== undefined) {
        mentoring_report.ment_report_mtr = req.files['appendix7'][0].path;
        console.log('submit Mentoring Report: ' + req.files['appendix7'][0].path + ', id: ' + req.session.userId);
        console.log("file upload success.");
      }
    }else if(req.params.PJId == 'AddReport8') {
      var query = "update mentoring_report set ment_report_mtr = '" + req.files['appendix8'][0].path+"'";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt8+"'";
      console.log('Update Mentoring Report: ' + req.files['appendix8'][0].path + ', id: ' + req.session.userId);
      if(req.files['appendix8'] !== undefined) {
        mentoring_report.ment_report_mtr = req.files['appendix8'][0].path;
        console.log('submit Mentoring Report: ' + req.files['appendix8'][0].path + ', id: ' + req.session.userId);
        console.log("file upload success.");
      }
    }else if(req.params.PJId == 'AddReport9') {
      var query = "update mentoring_report set ment_report_mtr = '" + req.files['appendix9'][0].path+"'";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt9+"'";
      console.log('Update Mentoring Report: ' + req.files['appendix9'][0].path + ', id: ' + req.session.userId);
      if(req.files['appendix9'] !== undefined) {
        mentoring_report.ment_report_mtr = req.files['appendix9'][0].path;
        console.log('submit Mentoring Report: ' + req.files['appendix9'][0].path + ', id: ' + req.session.userId);
        console.log("file upload success.");
      }
    }else if(req.params.PJId == 'AddReport10') {
      var query = "update mentoring_report set ment_report_mtr = '" + req.files['appendix10'][0].path+"'";
      query += " where prj_id = '"+req.body.PJId+"' and meeting_date = '"+req.body.MTDt10+"'";
      console.log('Update Mentoring Report: ' + req.files['appendix10'][0].path + ', id: ' + req.session.userId);
      if(req.files['appendix10'] !== undefined) {
        mentoring_report.ment_report_mtr = req.files['appendix10'][0].path;
        console.log('submit Mentoring Report: ' + req.files['appendix10'][0].path + ', id: ' + req.session.userId);
        console.log("file upload success.");
      }
    }
...
exports.postMentoringReport = (req, res) => {
  ...
  if(req.session.userInfo.userType=='student'){
    ...
    query += " where sti.std_id = '"+ req.session.userId+"'and t.prj_id = '"+ req.params.PJId+"' and t.team_id = sti.team_id; ";
  }
  else{
    query += "select t.* from team as t"
  ...
  query += "select * from agmt where use_yn = 1;";
    ...
    mysqlPool.pool.getConnection((err, connection) => {
      ...
      connection.query(query, (error, results, fields) => {
        ...
        //use results and fields
        res.render('pjmng/DGU521', {FinalLists: results, moment : moment, userInfo: req.session.userInfo});
      });
    });
};
//post Files
...
exports.postFinalLists = (req,res)=>{
  ...
  fileUpload(fileInfo).multipartForm(req, res, (err) => {
    ...
    if(req.params.formType == 'PostReport') {
      query += "update final_product set fin_report = '" + req.files['FinalReportFile'][0].path+ "',";
      if(req.body.RegisterReportCheck == "true") {
        console.log('submit Report: ' + req.files['FinalReportFile'][0].path + ', id: ' + req.session.userId);
    ...
    if(req.params.formType == 'PostProduct') {
      console.log('submit Product: ' + req.files['ProductFile'][0].path + ', id: ' + req.session.userId);
      query += "update final_product set product1 = '" + req.files['ProductFile'][0].path + "',";
    ...
    if(req.params.formType == 'PostVideo') {
      console.log('submit Video File: ' + req.files['VideoFile'][0].path + ', id: ' + req.session.userId);
      query += "update final_product set demo_vid = '" + req.files['VideoFile'][0].path + "',";
    ...
    if(req.params.formType == 'PostPatent') {
      console.log('submit Patent File: ' + req.files['PatentFile'][0].path + ', id: ' + req.session.userId);
      query += "update final_product set patent_doc = '" + req.files['PatentFile'][0].path + "',";
    ...
    if(req.params.formType == 'PostProgram') {
      console.log('submit Program File: ' + req.files['ProgramFile'][0].path + ', id: ' + req.session.userId);
      query += "update final_product set prg_regis = '" + req.files['ProgramFile'][0].path + "',";
    ...
    if(req.params.formType == 'PostThesis') {
      ...
      query += "update final_product set thesis_file = '" + req.files['ThesisFile'][0].path + "',";
    ...
    if(req.params.formType == 'PostOther1') {
      console.log('submit Other1 File: ' + req.files['Other1File'][0].path + ', id: ' + req.session.userId);
      query += "update final_product set other1 = '" + req.files['Other1File'][0].path + "',";
    ...
    if(req.params.formType == 'PostOther2') {
      console.log('submit Other2 File: ' + req.files['Other2File'][0].path + ', id: ' + req.session.userId);
      query += "update final_product set other2 = '" + req.files['Other2File'][0].path + "',";
    ...
    if(req.params.formType == 'PostOther3') {
      console.log('submit Other3 File: ' + req.files['Other3File'][0].path + ', id: ' + req.session.userId);
      query += "update final_product set other3 = '" + req.files['Other3File'][0].path + "',";
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query, (error, results, fields) => {
      ...
      if(results.length > 0) {
        ...
        res.render('pjmng/DGU513', {TeamInfo: results, moment : moment, userInfo: req.session.userInfo});
      } else {
        res.redirect('/');
      }
    });
...
exports.postMentoringReport2 = (req, res) => {
  ...
  fileUpload(fileInfo).multipartForm(req, res, (err) => {
    ...
    if(req.files['appendix'] !== undefined) {
      ...
      console.log('submit Mentoring Report: ' + req.files['appendix'][0].path + ', id: ' + req.session.userId);
      console.log("file upload success.");
    }
    //get connection from pool
    mysqlPool.pool.getConnection((err, connection) => {
