...
exports.downloadProjectFile = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var query =
      "select p.prj_id,p.prj_name, p.appendix, fp.fin_report, fp.product1, fp.demo_vid, fp.patent_doc, fp.prg_regis, fp.thesis_file, fp.other1, fp.other2, fp.other3 from project as p ";
    query +=
      "left outer join (select fp.prj_id , fp.team_id, fp.fin_report, fp.product1, fp.demo_vid, fp.patent_doc, fp.prg_regis, fp.thesis_file, fp.other1, fp.other2, ";
    query +=
      "fp.other3 from final_product as fp, project as p1 where p1.prj_id = fp.prj_id) as fp on p.prj_id = fp.prj_id ";
    query += "where p.prj_id in " + query_temp + " order by p.prj_id;";

    query +=
      "select p.prj_id, sti.std_resume from project as p, std_team_info as sti, team as t";
    query +=
      " where p.prj_id in " +
      query_temp +
      " and p.prj_id = t.prj_id and t.team_id = sti.team_id order by p.prj_id;";

    query +=
      "select m.prj_id, m.ment_report_mtr, m.ment_report_std from mentoring_report as m";
    ...
    connection.query(query, null, (error, results, fields) => {
      ...
      var topDirName =
        "/home/dev/sd/public/transferedFileDownload/" +
        moment().format("YYMMDD_kkmmss") +
        "_transferedProject";
      ...
      for (var i in fileArr) {
        ...
        for (var j in fileArr[i]) {
          ...
          destPath =
            topDirName + "/" + dirName + "/" + fileArr[i][j].split("/")[2];
...
exports.downloadMentorProjectFile = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var query =
      "select * from project where settings_id = '" + req.params.item_id + "';";
    connection.query(query, null, (error, results, fields) => {
      connection.release();
...
exports.downloadProjectPlanFile = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var query =
      "select ppr.* from project_plan_report as ppr where prj_id in (select prj_id from project where settings_id = '" +
      req.params.item_id +
      "');";
    connection.query(query, null, (error, results, fields) => {
      connection.release();
...
exports.downloadMentoringReportFile = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var query =
      "select mr.* from mentoring_report as mr where prj_id in (select prj_id from project where settings_id = '" +
      req.params.item_id +
      "');";
    connection.query(query, null, (error, results, fields) => {
      connection.release();
...
exports.downloadFinalProductFileFile = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var query =
      "select fp.* from final_product as fp where prj_id in (select prj_id from project where settings_id = '" +
      req.params.item_id +
      "');";
    connection.query(query, null, (error, results, fields) => {
      connection.release();
...
exports.getPastListPage = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    //use connection
    var query =
      "select ads.settings_id, ads.prj_year, ads.prj_semes, ads.term_chk, ads.transfer_date from admin_settings as ads ";
    query += " where ads.transfer_date is not null";
    query += " order by prj_year desc, prj_semes desc; ";

    query +=
      "select distinct p.prj_id, ads.prj_year, ads.prj_semes, ads.term_chk, ads.transfer_date, p.prj_name, p.prj_id, ct_me.cnt as mentor_cnt, t.team_name, ";
    query +=
      "t.class_num, ins.major, ins.inst_name, me.company_name, me.mentor_name, me.phone_num ";
    query += "from admin_settings as ads, project as p ";
    query +=
      "left outer join (select count(*) as cnt, mr.prj_id from mentoring_report as mr, project as p1 where mr.prj_id = p1.prj_id group by p1.prj_id) as ct_me on p.prj_id = ct_me.prj_id ";
    query +=
      "left outer join (select t.team_name, t.class_num, t.prj_id from team as t, project as p2 where t.prj_id = p2.prj_id group by p2.prj_id) as t on p.prj_id = t.prj_id ";
    query +=
      "left outer join (select ins.inst_name, ins.inst_id, ins.major, ci.class_num from instructor as ins, class_info as ci where ins.inst_id = ci.inst_id) as ins on t.class_num = ins.class_num ";
    query +=
      "left outer join (select me.mentor_id, me.company_name , me.mentor_name, me.phone_num from mentor as me, project as p3 where p3.mentor_id = me.mentor_id ) as me on p.mentor_id = me.mentor_id ";
...
exports.getMovingListPage = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    //use connection
    var query =
      'select settings_id,prj_year, prj_semes, term_chk, DATE_FORMAT(transfer_date,"%Y-%m-%d %T") as transfer_date from admin_settings ';
...
exports.getSearchproject1 = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    query +=
      "select p.prj_name, t.team_name, t.prj_id, sti.*, st.major, st.email_ad, st.std_name, st.phone_num from project as p, team as t, std_team_info as sti, student as st";
    query +=
      " where p.use_yn = 0 and t.prj_id = p.prj_id and sti.team_id = t.team_id and p.prj_id = " +
      req.params.PJId +
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query, (error, results, fields) => {
      ...
      if (results.length > 0) {
        ...
        res.render("setting/EditSetting", {
          SettingInfo: results,
          moment: moment,
          userInfo: req.session.userInfo,
        });
...
exports.postDeleteSetting = (req, res) => {
  ...
  var query = "";
  query +=
    "delete from admin_settings where settings_id = '" +
    req.body.settingID +
    "' ; ";
...
exports.getClassListPage = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    query +=
      "where c.inst_id = i.inst_id and c.settings_id = s.settings_id and c.use_yn = 1 ";
    query +=
...
exports.getAddClassPage = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query1, null, (error, results1, fields) => {
      ...
      connection.release();
      //use results and fields
      res.render("setting/AddClass", {
        SettingInstList: results1,
        userId: req.session.userId,
        userType: req.session.userType,
        userInfo: req.session.userInfo,
        moment: moment,
...
exports.postAddClassPage = (req, res) => {
  ...
  for (var i = 0; i <= Class_num; i++) {
    ...
    query +=
      "insert into class_info (settings_id, class_num, class_name, inst_id, assis_id,  registrant, regis_date, amender, amend_date) ";
    query +=
      "values ('" +
      settings_id +
      "', '" +
...
exports.getEditClass = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var query =
      "Select adse.prj_year, adse.prj_semes, adse.term_chk, c.settings_id, c.class_num, c.class_name, c.inst_id, i.inst_name, i.major, c.assis_id, a.assis_name ";
    query += "From class_info c ";
    query +=
      "LEFT JOIN admin_settings as adse ON c.settings_id = adse.settings_id ";
    ...
    query +=
      "select sub_user_id from class_inst_authority where class_num = '" +
      req.params.classnum +
      "' and settings_id = '" +
      req.params.settings_id +
      "';";
    ...
    connection.query(query, (error, results, fields) => {
      ...
      if (results.length > 0) {
        ...
        res.render("setting/EditClass", {
          ...
          moment: moment,
          userInfo: req.session.userInfo,
          ClassInformation: results[0],
          ClassSubUserIdList: results[4],
        });
      } else {
        res.redirect("/");
      }
...
exports.postEditClass = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var query =
      "insert into class_inst_authority (class_num, settings_id, sub_user_id, registrant, regis_date) values ?";
    if (records.length > 0) {
    ...
    query +=
      " where class_num = '" +
      req.body.originClassNum +
      "' and settings_id = '" +
      req.body.originSettingsId +
      "'; ";
...
exports.deleteSubUser = (req, res) => {
  ...
  // console.log(req.body);
  var query =
    "delete from class_inst_authority where class_num = '" +
    req.body.class_num +
    "' and settings_id = '" +
    req.body.settings_id +
    "' and sub_user_id = '" +
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query, (error, results, fields) => {
      ...
      if (results.length > 0) {
        ...
        res.render("setting/ReportSample", {
          apdxInfo: results,
          moment: moment,
          userInfo: req.session.userInfo,
        });
...
exports.postSample = (req, res) => {
  ...
  fileUpload(fileInfo).multipartForm(req, res, (err) => {
    ...
    if (req.files["PrjAply"] !== undefined) {
      ...
      console.log(
        "Project Apply Sample file upload success." +
          req.files["PrjAply"][0].path
      );
    }
...
exports.postNotice = (req, res) => {
  ...
  fileUpload(fileInfo).multipartForm(req, res, (err) => {
    ...
    if (req.params.formtype == "image1") {
      query +=
        "update main_notice set image1 = '" + req.files["Image1"][0].path + "'";
      query += " where use_yn = 1";
    }
    if (req.params.formtype == "image2") {
      query +=
        "update main_notice set image2 = '" + req.files["Image2"][0].path + "'";
      query += " where use_yn = 1";
    }
    if (req.params.formtype == "image3") {
      query +=
        "update main_notice set image3 = '" + req.files["Image3"][0].path + "'";
      query += " where use_yn = 1";
    }
    if (req.params.formtype == "image4") {
      query +=
        "update main_notice set image4 = '" + req.files["Image4"][0].path + "'";
      query += " where use_yn = 1";
    }
    if (req.params.formtype == "image5") {
      query +=
        "update main_notice set image5 = '" + req.files["Image5"][0].path + "'";
      query += " where use_yn = 1";
    }
