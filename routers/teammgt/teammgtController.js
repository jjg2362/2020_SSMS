...
exports.getTeam = (req, res) => {
  ...
  var query =
    "select team_name from team where team_id = (select team_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and class_type = 1 and team_yn = 1 and use_yn = 1);";
  query +=
    "select team_name from team where team_id = (select team_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and class_type = 2 and team_yn = 1 and use_yn = 1);";
  query +=
    "select team_name from team where team_id = (select team_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and class_type = 3 and team_yn = 1 and use_yn = 1);";
  ...
  query +=
    "select team_name, team.settings_id, prj_year, prj_semes, term_chk from team, admin_settings ";
  query +=
    "where team_id in (select team_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
...
exports.postTeamCreate = (req, res) => {
  ...
  var checkQuery =
    "select std_id, class_type from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and class_type = " +
    req.body.teamCreateType +
    ";";
  checkQuery +=
    "select team_name from team where team_name = '" + req.body.teamName + "';";
  mysqlPool.pool.getConnection((err, connection) => {
    if (err) {
      //throw err;
      console.error("getConnection err : " + err);
    ...
    connection.query(checkQuery, (checkError, checkResults, checkFields) => {
      ...
      if (checkResults[0].length > 0) {
        ...
        var query =
          "insert into team(team_name, leader_id, settings_id) values('" +
          req.body.teamName +
          "', '" +
          req.session.userInfo.userId +
          "', '" +
          ...
          "');";
        query +=
          "insert into std_team_info(std_id, team_id, leader_id, team_yn, class_type, use_yn) values('" +
          req.session.userInfo.userId +
          "', LAST_INSERT_ID(), '" +
          req.session.userInfo.userId +
          "', 1, " +
...
exports.postTeamDelete = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var checkQuery =
      "select std_id, leader_id from std_team_info where team_id = (select team_id from std_team_info where std_id = '" +
      req.session.userInfo.userId +
      "' and class_type = " +
      req.body.teamDeleteType +
      " and team_yn = 1 and use_yn = 1) and team_yn = 1 and use_yn = 1;";
    ...
    connection.query(checkQuery, (checkError, checkResults, checkFields) => {
      ...
      if (checkResults[0].leader_id != req.session.userInfo.userId) {
        addQuery +=
          "delete from std_team_info where std_id = '" +
          req.session.userInfo.userId +
          "' and class_type = " +
          req.body.teamDeleteType +
          " and team_yn = 1 and use_yn = 1;";
        ...
        addQuery +=
          "delete from team where team_id = (select team_id from std_team_info where std_id = '" +
          req.session.userInfo.userId +
          "' and class_type = " +
          req.body.teamDeleteType +
          " and team_yn = 1 and use_yn = 1) and use_yn = 1;";
        addQuery +=
          "delete from std_team_info where std_id = '" +
          req.session.userInfo.userId +
          "' and class_type = " +
...
exports.getTeamMember = (req, res) => {
  ...
  var query =
    "select leader_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and class_type = 1;";
  query +=
    "select leader_id from std_team_info where std_id = '" +
  ...
  query +=
    "select std_id, std_name, major from student where std_id in (select std_id from std_team_info where leader_id = '" +
    req.session.userInfo.userId +
    "' and class_type='1' and team_yn = 0) and std_id <> '" +
    req.session.userInfo.userId +
    "';";
  query +=
    "select std_id, std_name, major from student where std_id in (select std_id from std_team_info where leader_id = '" +
    req.session.userInfo.userId +
    "' and class_type='2' and team_yn = 0) and std_id <> '" +
    req.session.userInfo.userId +
    "';";
  query +=
    "select std_id, std_name, major from student where std_id in (select std_id from std_team_info where leader_id = '" +
    req.session.userInfo.userId +
    "' and class_type='3' and team_yn = 0) and std_id <> '" +
    req.session.userInfo.userId +
    "';";
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query, (error, results, fields) => {
      ...
      // console.log(results);
      //use results and fields
      res.render("teammgt/DGU251", {
        myTeamLeader: results,
        userInfo: req.session.userInfo,
        teamType: results[5]
      });
    });
...
exports.postTeamSearch = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var query =
      "select std_id, std_name, major from student where std_name = '" +
      req.body.stdName +
      "';";
    ...
    connection.query(query, (error, results, fields) => {
      ...
      if (results.length == 0) {
        ...
        for (var i in results) {
          addQuery +=
            "select team_id, team_yn from std_team_info where std_id = '" +
            results[i].std_id +
            "' and use_yn = 1 and class_type = " +
            req.body.teamSearchType +
            ";";
        }
        ...
        connection.query(addQuery, (addError, addResults, addFields) => {
          ...
          for (var i in results) {
            info[results[i].std_id] = {
              name: results[i].std_name,
...
exports.postTeamAdd = (req, res) => {
  ...
  var query =
    "insert into std_team_info(std_id, team_id, leader_id, team_yn, class_type, use_yn) values('" +
    req.body.stdId +
    "', (select b.team_id from (select team_id from std_team_info as a where a.std_id = '" +
    req.session.userInfo.userId +
    "' and a.class_type = " +
    req.body.addType +
    ") as b), '" +
...
exports.getMemberOfMyInvitation = (req, res) => {
  ...
  query +=
    "where std_id in (select std_id from std_team_info where leader_id = '" +
    req.session.userInfo.userId +
    "' and class_type='" +
    req.query.classType +
    "' and team_yn = 0) ";
...
exports.getMyTeam = (req, res) => {
  ...
  var query =
    "select std_id, std_name, major from student where std_id = (select leader_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and team_yn = 0 and class_type = 1);";
  query +=
    "select std_id, std_name, major from student where std_id = (select leader_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and team_yn = 0 and class_type = 2);";
  query +=
    "select std_id, std_name, major from student where std_id = (select leader_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and team_yn = 0 and class_type = 3);";

  // results[3]~[5]: 나의팀(팀명)
  query +=
    "select team_name from team where team_id = (select team_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and team_yn = 1 and class_type = 1);";
  query +=
    "select team_name from team where team_id = (select team_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and team_yn = 1 and class_type = 2);";
  query +=
    "select team_name from team where team_id = (select team_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and team_yn = 1 and class_type = 3);";
  ...
  // results[9]~[11]: 나의팀(팀)
  query +=
    "select student.std_id, std_name, major, std_team_info.std_resume from student left join std_team_info on student.std_id = std_team_info.std_id and std_team_info.class_type = 1 where student.std_id in (select std_id from std_team_info where team_id = (select team_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and team_yn = 1 and class_type = 1) and std_id <> leader_id and team_yn = 1);";
  query +=
    "select student.std_id, std_name, major, std_team_info.std_resume from student left join std_team_info on student.std_id = std_team_info.std_id and std_team_info.class_type = 2 where student.std_id in (select std_id from std_team_info where team_id = (select team_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and team_yn = 1 and class_type = 2) and std_id <> leader_id and team_yn = 1);";
  query +=
    "select student.std_id, std_name, major, std_team_info.std_resume from student left join std_team_info on student.std_id = std_team_info.std_id and std_team_info.class_type = 3 where student.std_id in (select std_id from std_team_info where team_id = (select team_id from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and team_yn = 1 and class_type = 3) and std_id <> leader_id and team_yn = 1);";
  // results[12]~[14]
  query +=
    "select std_resume from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and class_type = 1 and std_resume is not null;";
  query +=
    "select std_resume from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and class_type = 2 and std_resume is not null;";
  query +=
    "select std_resume from std_team_info where std_id = '" +
    req.session.userInfo.userId +
    "' and class_type = 3 and std_resume is not null;";
  // results[15]
  query += "select prj_aply_apdx from apdx_file_info where use_yn = 1;";
...
exports.postPCFile = (req, res) => {
  ...
  fileUpload(fileInfo).multipartForm(req, res, err => {
    ...
    mysqlPool.pool.getConnection((err, connection) => {
      ...
      var query =
        "update std_team_info set std_resume = '" +
        uploadPCFIle.std_resume +
        "' where std_id = '" +
        req.session.userInfo.userId +
        "' and class_type = " +
        req.body.pfType +
...
exports.postTeamInvite = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    if (req.params.type == "accept") {
      query +=
        "update std_team_info set team_yn = 1 where std_id = '" +
        req.session.userInfo.userId +
        "' and leader_id = '" +
        req.body.acId +
        "' and class_type = " +
...
exports.getMyInvitation = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var query =
      "select std_id, std_name, major from student where std_id in (select leader_id from std_team_info where std_id = '" +
      userId +
      "' and team_yn = 0 and class_type='" +
      req.query.classType +
      "');";
...
exports.getMyTeamInfo = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    /* 팀원 */
    query +=
      "select student.std_id, std_name, major, std_team_info.std_resume from student left join std_team_info on student.std_id = std_team_info.std_id and std_team_info.class_type = '" +
      classType +
      "' where student.std_id in (select std_id from std_team_info where team_id = (select team_id from std_team_info where std_id = '" +
      userId +
      "' and team_yn = 1 and class_type = '" +
      classType +
      "') and std_id <> leader_id and team_yn = 1);";
...
exports.getTeamSelectPj = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var pjQuery =
      "select prj_id, prj_name from project left join admin_settings on project.settings_id = admin_settings.settings_id where mentor_id = '" +
      req.session.userInfo.userId +
      "' and project.use_yn = 1 and prj_id not in (select distinct prj_id from team where use_yn = 1 and prj_id is not null) and now() < mtch_cls_date;";
    ...
    connection.query(pjQuery, (pjError, pjResults, pjFields) => {
      ...
      if (pjResults.length == 0) {
        ...
        for (var i in pjResults) {
          teamQuery +=
            "select team_id, team_name from team where team_id in (select distinct team_id from project_cart where project1 = '" +
            pjResults[i].prj_id +
            "' and use_yn = 1);";
        }
        ...
        connection.query(teamQuery, (teamError, teamResults, teamFields) => {
          ...
          if (pjResults.length == 1) {
            info[pjResults[0].prj_id] = {
              prjName: pjResults[0].prj_name,
              want1: teamResults
            };
          } else {
            for (var i in pjResults) {
              info[pjResults[i].prj_id] = {
                prjName: pjResults[i].prj_name,
                want1: teamResults[i]
              };
            }
          }
...
exports.postTeamSelectStd = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var stdQuery =
      "select a.std_id, a.std_name, a.major, b.leader_id, b.std_resume from student as a, std_team_info as b where b.team_id = '" +
      req.body.t_id +
...
exports.postTeamAccept = (req, res) => {
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    var query =
      ...
      "';";
    query +=
      "insert into final_product(team_id, prj_id, mentor_id, use_yn) values('" +
      req.body.t_id +
      "', '" +
      req.body.p_id +
      "', '" +
...
exports.getAllTeamInfoma = (req, res) => {
  ...
  //use connection
  var query =
    "select d.class_num, d.team_name, a.std_id , d.leader_id, a.std_grade , a.std_name , a.major ,   ";
  query +=
    "a.email_ad ,a.phone_num , b.std_resume ,  p.prj_name, i.class_name ";
  query += "from student as a ";
  query += "left join std_team_info as b on a.std_id = b.std_id ";
  query += "left join team as d on d.team_id = b.team_id ";
  query += "left join project as p on p.prj_id=d.prj_id ";
  query +=
    "left join (select class_num, class_name, inst_id from class_info) as i on d.class_num = i.class_num ";
  query +=
    "where i.inst_id ='" +
    req.session.userId +
    "' order by d.class_num, b.team_id desc;";
...
exports.getAllTeamInfoAS = (req, res) => {
  ...
  query += "left join project as p on p.prj_id=d.prj_id ";
  query +=
    "left join (select class_num, class_name, assis_id from class_info) as i on d.class_num = i.class_num ";
  query +=
    "where i.assis_id ='" +
    req.session.userId +
    "' order by d.class_num, b.team_id desc;";
...
exports.getAllProjectInfo = (req, res) => {
  ...
  //use connection
  var query =
    "select p.prj_id, p.prj_name, p.mentor_id , m.mentor_name, m.company_name, m.phone_num, m.email_ad,t.team_name as fixed_team, ";
  query +=
    "pj1_team.teams as team1, pj2_team.teams as team2, pj3_team.teams as team3 ";
  query +=
    "from project as p left join mentor as m on p.mentor_id = m.mentor_id and p.use_yn='1' ";
  query += "left join team as t on t.prj_id = p.prj_id ";
  query +=
    "left join (select pjcart.project1, group_concat(t.team_name separator ', ') as teams from project_cart as pjcart ";
  query +=
    "left outer join team as t on t.team_id = pjcart.team_id group by pjcart.project1) as pj1_team ";
  query += "on pj1_team.project1 = p.prj_id ";
  query +=
    "left join (select pjcart.project2, group_concat(t.team_name separator ', ') as teams from project_cart as pjcart ";
  query +=
    "left outer join team as t on t.team_id = pjcart.team_id group by pjcart.project2) as pj2_team ";
  query += "on pj2_team.project2 = p.prj_id ";
  query +=
    "left join (select pjcart.project3, group_concat(t.team_name separator ', ') as teams from project_cart as pjcart ";
  query +=
    "left outer join team as t on t.team_id = pjcart.team_id group by pjcart.project3) as pj3_team ";
...
exports.getTeamPjInfo = (req, res) => {
  ...
  query +=
    "where t.use_yn ='1' and t.prj_id is not null and p.prj_id=t.prj_id ";
...
exports.postTeamPjMatching = (req, res) => {
  ...
  query +=
    "prj_id = '" + PjMentor[0] + "', mentor_id = '" + PjMentor[1] + "', ";
  query +=
    " amend_date ='" +
...
exports.getTeamPjDelete = (req, res) => {
  ...
  var query =
    "Update team set prj_id = NULL, mentor_id = NULL where team_id =" +
    req.params.teamId +
    ";";
...
exports.getTeamClassInfo = (req, res) => {
  ...
  var query =
    "select t.* from team as t where t.use_yn='1' order by t.team_name ; ";

  query +=
    "select cinfo.*, i.major, i.inst_name from class_info as cinfo, instructor as i ";
  ...
  query +=
    "left join class_info as cinfo on t.settings_id = cinfo.settings_id and t.class_num = cinfo.class_num ";
  query += "left join instructor as i on  cinfo.inst_id = i.inst_id ";
  query +=
    "where t.use_yn = '1' and t.class_num is not null order by t.amend_date desc;";
  ...
  mysqlPool.pool.getConnection((err, connection) => {
    ...
    connection.query(query, (error, results, fields) => {
      ...
      //use results and fields
      res.render("teammgt/DGU261", {
        TeamClassName: results,
        userInfo: req.session.userInfo
      });
    });
  });
